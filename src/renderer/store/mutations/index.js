...
export default {
  initLocalData(state){
    state.datastore.db.session.find({})
    .sort({
      updateTime: -1,
      id:1
    })
    .exec((err, ret) => {
      state.sessionlist=ret
    })
    /* state.datastore.db.usersInfo.find({}, function (err, users) {
      if(!users) return;
  ...
  upsyncdone(state,status){
    state.issyncdone=status;
  },
  updateNimStateLeave(state,obj){
    state.isLeave=obj.isLeave;
    state.leaveReply=obj.message||'';
  },
  ...
  updateUserInfo (state, users) {
    ...
    users.forEach(user => {
      let account = user.account
      if (account) {
        /* state.datastore.db.usersInfo.update({account:account},user,{upsert:true},function(err, numReplaced){
  ...
  updateSessions (state, sessions,callback) {
    const nim = state.nim;
    sessions.map((item)=>{
        if(item.scene=='team'&&item.lastMsg.attach&&item.lastMsg.attach.type=="dismissTeam"){
          state.datastore.db.session.remove({id:item.id},{})
        }else{
          state.datastore.db.session.update({id:item.id},item,{upsert:true},function(err, numReplaced){
    ...
    let arr=_.sortBy(state.sessionlist, ['updateTime', 'id']);
    state.sessionlist=_.reverse(arr); */
