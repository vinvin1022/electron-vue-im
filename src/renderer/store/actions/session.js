...
export function onSessions (sessions) {
  ...
  store.commit('updateSessions', sessions,true)
}
...
export function resetCurrSession ({state, commit,dispatch}) {
  const nim = state.nim
  nim&&nim.resetCurrSession()
  commit('updateCurrSessionMsgs', {
    type: 'destroy'
  })
  dispatch("resetNoMoreHistoryMsgs");
}
