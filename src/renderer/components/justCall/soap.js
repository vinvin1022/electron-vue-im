 ...
 * Licensed under GPLv2: https://github.com/gtathub/js-soap-client.git
 ...
 {
    let pl = new SOAPClientParameters();
    for(let key in parameters){
        pl.add(key,parameters[key]);
    }
    parameters=pl;
     if(async)
 ...
 {
     var urlEndPoind =  SOAPClient._getEndPointFromWsdl(wsdl);
     var ns = (typeof wsdl.documentElement.attributes["name"] == "undefined") ? wsdl.documentElement.attributes.getNamedItem("name").nodeValue : wsdl.documentElement.attributes["name"].value;
     var urn = wsdl.documentElement.attributes["name"].value;
     // build SOAP request
     var sr =
         ...
         ">" +
         (SOAPClient.auth?"<soapenv:Header><AuthHeader xmlns=\"" + ns + "\">" +
             "<Username>"+SOAPClient.authUser+"</Username>" +
             "<Password>"+SOAPClient.authPass+"</Password>" +
             "</AuthHeader></soapenv:Header>":"") +
         "<soapenv:Header\/>"+
         "<soapenv:Body>" +
         //(SOAPClient.explicitNS?"<urn:" + method + " soap:encodingStyle=\"http://schemas.xmlsoap.org/soap/encoding/\">":"<" + method + " xmlns=\"" + ns + "\">") +
         "<urn:" + method + " soapenv:encodingStyle=\"http://schemas.xmlsoap.org/soap/encoding/\">"+ // soap:encodingStyle=\"http://schemas.xmlsoap.org/soap/encoding/\">" +
         parameters.toXml() +
         //(SOAPClient.explicitNS?"</urn:" + method + ">":"</" + method + ">") +
         "</urn:" + method + ">" +
         "</soapenv:Body></soapenv:Envelope>";
 
     var xmlHttp = SOAPClient._getXmlHttp();
     if (SOAPClient.userName && SOAPClient.password){
         xmlHttp.open("POST", urlEndPoind, async, SOAPClient.userName, SOAPClient.password);
         // Some WS implementations (i.e. BEA WebLogic Server 10.0 JAX-WS) don't support Challenge/Response HTTP BASIC, so we send authorization headers in the first request
         xmlHttp.setRequestHeader("Authorization", "Basic " + SOAPClient._toBase64(SOAPClient.userName + ":" + SOAPClient.password));
     }
     else{
        xmlHttp.open("POST", urlEndPoind, async);
     }
 ...
 SOAPClient._onSendSoapRequest = function(method, async, callback, wsdl, req)
 {
    ...
    var tryTags = ["ns:"+method + "Response",method + "Response"];
    ...
    //   nd=SOAPClient._getElementsByTagName(req.responseXML,  "ns:"+method + "Response");
     // var secondArg=['call-id']
     if(nd.length == 0)
     {
 ...
 SOAPClient._getEndPointFromWsdl = function(wsdl)
 {
     //alert('_getEndPointFromWsdl');
     var wsdlTypes = new Array();
     // IE
     var ell = wsdl.getElementsByTagName("SOAP:address");
     var useNamedItem = true;
     // MOZ
     if(ell.length == 0)
     {
         ell = wsdl.getElementsByTagName("address");
         useNamedItem = false;
     }
     if(ell.length == 0)
     {
         alert('problem while loading WSDL');
     }
     ...
     if(useNamedItem)
     {
         //alert(ell[0].getAttribute("location"));
         theEndPoint = ell[0].getAttribute("location");
     ...
     else
     {
         ...
         //ell[i].attributes["type"].value;
     }
     //alert(ell[0].getAttribute("location"));
 
     if(theEndPoint.indexOf('https')>-1){
        theEndPoint='https'+theEndPoint.split('https')[1];
     }
     return theEndPoint;
 }
